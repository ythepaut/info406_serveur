...
if (!empty($requestData['token'])) {
    ...
    if (PermissionManager::getInstance($jwtConfig['key'])->isTokenValid($requestData['token'])) {
        ...
        foreach ($rooms as $room) {
            array_push($list, array($room->getNumber() => array("number" => $room->getNumber(),
                                                            "type" => $room->getType(),
                                                            "seats" => $room->getSeats(),
                                                            "computers" => $room->getComputers())));
        }

        $response = new Response(ResponseEnum::SUCCESS_ROOMS_LISTED, array("rooms" => $list), ResponseType::JSON);
